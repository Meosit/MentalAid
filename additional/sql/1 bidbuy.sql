...
CREATE UNIQUE INDEX `username_UNIQUE`
  ON `bidbuy`.`user` (`username` ASC);

CREATE UNIQUE INDEX `email_UNIQUE`
  ON `bidbuy`.`user` (`email` ASC);
...
CREATE TABLE IF NOT EXISTS `bidbuy`.`bid` (
  ...
  COMMENT 'ID of a lot this bid corresponds to',
...
CREATE INDEX `client_id_FK_INDEX`
  ON `bidbuy`.`bid` (`client_id` DESC);

CREATE INDEX `lot_id_FK_INDEX`
  ON `bidbuy`.`bid` (`lot_id` DESC);
...
CREATE TABLE IF NOT EXISTS `bidbuy`.`lot` (
  ...
  COMMENT 'ID of the user, who created this lot',
  `leader_bid_id`     INT UNSIGNED                     NULL     DEFAULT NULL
  COMMENT 'Last bid for this lot.\nThis approach longer in bid inserting, but faster while selecting winner of the lot. \nAlso lot can determine transaction record itself.',
  `auction_type`      TINYINT(0) UNSIGNED              NOT NULL DEFAULT 1
  COMMENT 'Represents type of auction, at the current moment there are three types: \n1 - english auction\n2 - reversive auction\n3 - auction-lottery\n(tinyint according this article http://komlenic.com/244/8-reasons-why-mysqls-enum-data-type-is-evil/ )',
  `min_price`         DECIMAL(13, 4) UNSIGNED ZEROFILL NOT NULL
  COMMENT 'Represents minimum price of a lot, which setting up by user',
  `current_price`     DECIMAL(13, 4) UNSIGNED ZEROFILL NOT NULL
  COMMENT 'Represents price of lot at the current moment',
...
CREATE INDEX `owner_id_FK_INDEX`
  ON `bidbuy`.`lot` (`owner_id` ASC);

CREATE INDEX `name_INDEX`
  ON `bidbuy`.`lot` (`name` ASC);
...
CREATE TABLE IF NOT EXISTS `bidbuy`.`transaction` (
  ...
  COMMENT 'ID of owner of the lot that is transferred',
  `customer_id` INT UNSIGNED NOT NULL
  COMMENT 'ID of client who have winning bid on lot that is transferred',
  PRIMARY KEY (`lot_id`),
...
CREATE INDEX `customer_id_FK_INDEX`
  ON `bidbuy`.`transaction` (`customer_id` ASC);
...
CREATE TRIGGER `bidbuy`.`user_BEFORE_INSERT`
...
CREATE TRIGGER `bidbuy`.`user_BEFORE_UPDATE`
...
CREATE TRIGGER `bidbuy`.`bid_BEFORE_INSERT`
...
CREATE TRIGGER `bidbuy`.`lot_BEFORE_INSERT`
...
CREATE TRIGGER `bidbuy`.`lot_BEFORE_UPDATE`
...
CREATE TRIGGER `bidbuy`.`transaction_BEFORE_INSERT`
...
CREATE TRIGGER `bidbuy`.`bid_AFTER_INSERT`
AFTER INSERT ON `bid`
FOR EACH ROW
  BEGIN
    UPDATE `lot`
